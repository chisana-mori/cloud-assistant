...
function kindAccent(kind: StepView['kind']) {
    switch (kind) {
        ...
        case 'systemNote':
            return 'border-rose-300/60 bg-rose-50/60';
        default:
        ...
        <div className={`my-3 rounded-2xl border-l-4 border border-slate-200 dark:border-slate-800 shadow-sm ${kindAccent(step.kind)} dark:bg-[#2a2a2a]`}>
            <div className="p-4">
                ...
                {step.kind === 'systemNote' && (
                    <div className="mt-1 text-sm rounded-xl px-3 py-2 bg-rose-50 text-rose-700 border border-rose-200/60">
                        <div className="font-semibold">{meta.summary ?? 'System Error'}</div>
                        {meta.details && (
                            <details className="mt-2">
                                <summary className="text-xs font-semibold text-rose-600 cursor-pointer">
                                    Details
                                </summary>
                                <pre className="mt-2 text-xs whitespace-pre-wrap font-mono text-rose-800 bg-white/70 border border-rose-200/60 rounded p-2 max-h-56 overflow-y-auto">
                                    {meta.details}
                                </pre>
                            </details>
                        )}
